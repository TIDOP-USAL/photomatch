...
using namespace cv;
FLANNMatcherProcess::FLANNMatcherProcess(cv::Mat &cvImgLeft,cv::Mat &cvImgRight,std::vector<cv::KeyPoint> &cvkeyLeft,std::vector<cv::KeyPoint> &cvkeyRight,cv::Mat &cvDescLeft,cv::Mat &cvDescRight,std::vector<cv::DMatch> &correctMatches,std::vector<cv::DMatch> &wrongMatches,bool haveGroundTruth,cv::Mat &groundTruth,QString leftImageName,QString rightImageName):
