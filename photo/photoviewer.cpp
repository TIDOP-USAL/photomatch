...
//    pix->setFlag(QGraphicsItem::ItemIsMovable);
    ...
    editingPoints = false;
    maskItem = NULL;
    originPointItem = new CrosshairGraphicsItem(new PW2dPoint(0,0,"0,0"));
...
//            PW2dPoint *point = new PW2dPoint(transPoint.x(),transPoint.y(),"Px");
...
//            PW2dPoint *point = new PW2dPoint(transPoint.x(),transPoint.y(),currentControlPointsPW.at(i)->getName());
//            CrosshairGraphicsItem *crossItem = new CrosshairGraphicsItem(point);

            PW2dPoint *point = new PW2dPoint(transPoint.x(),transPoint.y(),currentControlPointsPW.at(i)->getName());
            CrosshairGraphicsItem *crossItem = new CrosshairGraphicsItem(new PW2dPoint(0,0,currentControlPointsPW.at(i)->getName()));
    ...
    if (editing){
        ...
        for (int i = 0; i<scene.items().count(); i++){
            if(scene.items().at(i)->type() == 4){
                ...
                connect(item,SIGNAL(pointChanged(PW2dPoint*,QPointF)),this, SLOT(onPointItemMouseRelease(PW2dPoint*,QPointF)));
            }
        }
    }
    else{
...
void PhotoViewer::onPointItemMouseRelease(PW2dPoint *point, QPointF pos)
{
    QTransform trans;
