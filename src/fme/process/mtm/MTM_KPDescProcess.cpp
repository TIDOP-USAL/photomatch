...
MTM_KPDescProcess::MTM_KPDescProcess(cv::Mat &cvImgLeft,cv::Mat &cvImgRight, std::vector<cv::KeyPoint> &cvkeyLeft,std::vector<cv::KeyPoint> &cvkeyRight,std::vector<std::vector<cv::DMatch>> &matchesL2R,std::vector<std::vector<cv::DMatch>> &matchesR2L,int sizePatch,int nBins, const QString &leftImageName, const QString &rightImageName):
...
void MTM_KPDescProcess::run()
{
  ...
  for (int i = 0; i < left_max; i++)
  {
    ...
    {
      threads.push_back(new std::thread(&calculate_distmap, patchs_norm_left[i], i, patchs_norm_right, steps, j*stepThread, ((j + 1)*stepThread) - 1, matchesltor_aux));
    }
    threads.push_back(new std::thread(&calculate_distmap, patchs_norm_left[i], i, patchs_norm_right, steps, (nThreads - 1)*stepThread, right_max - 1, matchesltor_aux));
  ...
  for (int i = 0; i < right_max; i++)
  {
    ...
    for (unsigned j = 0; j < nThreads - 1; j++)
    {
      threads.push_back(new std::thread(&calculate_distmap, patchs_norm_right[i], i, patchs_norm_left, steps, j*stepThread, ((j + 1)*stepThread) - 1, matchesltor_aux));
    }
    threads.push_back(new std::thread(&calculate_distmap, patchs_norm_right[i], i, patchs_norm_left, steps, (nThreads - 1)*stepThread, left_max - 1, matchesltor_aux));
