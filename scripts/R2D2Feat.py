...
def extract_features(image, features, top_k, scale_f, min_size, max_size, min_scale, max_scale, reliability_thr, repeatability_thr, gpu):

    iscuda = common.torch_set_gpu(gpu)
	
    # load the network...
    model_file = os.path.dirname(os.path.abspath(__file__)) + '/models/r2d2_WAF_N16.pt'
    ...
    print(f"Saving {len(idxs)} keypoints to {features}")
...
if __name__ == "__main__":
    ...
    extract_features(args.image, args.features, args.top_k, args.scale_f, args.min_size, args.max_size, args.min_scale, args.max_scale, args.reliability_thr, args.repeatability_thr, args.gpu)
